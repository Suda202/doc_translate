...
2. 使用 AskUserQuestion 询问是否沿用，**展示所有已保存的配置项**：
   - 翻译服务：如 Kimi + DeepSeek
   - 目标语言：如 简体中文
   - 术语库：如 default, tech, programming（显示具体名称）
   - 自动提取术语：开/关
   - 页面输出模式：如 仅选中页面
   - 富文本翻译：如 仅高级模型开启
   - 兼容模式：开/关
...
3. **复制到允许访问的目录**：Playwright MCP 只能访问项目目录（如 ~/project/coding/doc_translate/），需要先将文件复制到该目录
4. **检查配置**：读取 ~/.config/doc_translate/config.json
   - 无配置：上传文件后在网站上选择翻译配置
   - 有配置：使用 AskUserQuestion 询问是否沿用，**展示所有已保存的配置项**：
     - 翻译服务、目标语言、术语库、自动提取术语、导出格式
5. **翻译流程**：
